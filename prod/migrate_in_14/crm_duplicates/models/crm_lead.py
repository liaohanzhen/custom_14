...
class crm_lead(models.Model):
    """
    Override to add duplicates metrics and methods
    """
    _inherit = "crm.lead"
    ...
    def _compute_duplicates_count(self):
         ...
         * search_count
        """
        fields = self.env.user.sudo().company_id.duplicate_fields_lead_soft
        for record in self:
            domain = record._construct_domain(fields=fields, char_operator="ilike")
            duplicates_count = domain and self.sudo().search_count(domain) or 0
    ...
    def write(self, vals):
        ...
        for record in self:
            ...
            duplicate_leads_recordset = record._return_duplicated_records(fields_names="duplicate_fields_lead")
            if duplicate_leads_recordset:
    ...
    def merge_opportunity(self, user_id=False, team_id=False):
        ...
        """
        context = {'inhibit_duplication_warning': True,}
    ...
    def open_duplicates(self):
        ...
        return {
                ...
                'view_id': False,
                'res_model': 'crm.lead',
                'domain': str([('id', 'in', duplicates.ids + self.ids)]),
                'type': 'ir.actions.act_window',
                'nodestroy': True,
                'target': 'current',
                'context': {'form_view_ref': 'crm_duplicates.crm_case_form_view_oppor',},
        }
