...
class SmsCompose(models.Model):
    ...
    model = fields.Char()
    sms_template_id = fields.Many2one('sms.frame.template', string="Template")
    from_mobile_id = fields.Many2one('sms.number', required=True,
                                     string="From Mobile")
    ...
    def _onchange_sms_template_id(self):
        """Prefills from mobile, sms_account and sms_content but allow them
        to manually change the content after"""
        if self.sms_template_id.id != False:
            sms_rendered_content = self.env['sms.frame.template'].render_template(
                self.sms_template_id.template_body,
                self.sms_template_id.model_id.model, self.record_id)

            self.from_mobile_id = \
                self.sms_template_id.from_mobile_verified_id.id
...
#     @api.multi
    def send_entity(self):
        ...
        self.ensure_one()
        gateway_model = self.from_mobile_id.account_id.account_gateway_id\
            .gateway_model_name
        if self.delivery_time:
            ...
            sms_message = self.env['sms.message'].create(
                {'record_id': self.record_id, 'model_id': my_model[0].id,
                 ...
                 'status_string': '-', 'direction': 'O',
                 'message_date': self.delivery_time, 'status_code': 'queued',
            ...
            my_sms = self.from_mobile_id.account_id.send_message(
                self.from_mobile_id.mobile_number, self.to_number,
                self.sms_content, self.model, self.record_id, self.media_id,
