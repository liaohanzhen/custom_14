...
class SmsTemplate(models.Model):
    ...
    model = fields.Char(related="model_id.model",
                        string='Related Document Model', store=True,
                        readonly=True)
    template_body = fields.Text('Body', translate=True,
                                help="Plain text version of the message ("
                                     "placeholders may be used here)")
    sms_from = fields.Char(string='From (Mobile)',
                           help="Sender mobile number (placeholders may be "
                                "used here). If not set, the default value "
                                "will be the author's mobile number.")
    sms_to = fields.Char(string='To (Mobile)',
                         help="To mobile number (placeholders may be used "
                              "here)")
    account_gateway_id = fields.Many2one('sms.account', string="Account")
    model_object_field_id = fields.Many2one('ir.model.fields', string="Field",
                                            help="Select target field from "
                                                 "the related document "
                                                 "model.\nIf it is a "
                                                 "relationship field you "
    ...
    sub_object_id = fields.Many2one('ir.model', string='Sub-model',
                                    readonly=True,
                                    help="When a relationship field is "
                                         "selected as first field, "
                                         "this field shows the document "
                                         "model the relationship goes to.")
    sub_model_object_field_id = fields.Many2one('ir.model.fields',
                                                string='Sub-field',
    ...
    null_value = fields.Char(string='Default Value',
                             help="Optional value to use if the target field "
                                  "is empty")
    copyvalue = fields.Char(string='Placeholder Expression',
                            help="Final placeholder expression, "
                                 "to be copy-pasted in the desired template "
                                 "field.")
    lang = fields.Char(string='Language',
                       help="Optional translation language (ISO code) to "
                            "select when sending out an email. If not set, "
                            "the english version will be used. This should "
                            "usually be a placeholder expression that "
                            "provides the appropriate language, e.g. ${"
    ...
    def _onchange_model_object_field_id(self):
        if self.model_object_field_id.relation:
            self.sub_object_id = self.env['ir.model'].search(
                [('model', '=', self.model_object_field_id.relation)])[0].id
        else:
    ...
    def send_sms(self, template_id, record_id):
        """Send the sms using all the details in this sms template,
        using the specified record ID"""
        sms_template = self.env['sms.frame.template'].browse(int(template_id))
        rendered_sms_template_body = self.env['sms.frame.template'].render_template(
            sms_template.template_body, sms_template.model_id.model, record_id)
        rendered_sms_to = self.env['sms.frame.template'].render_template(
            sms_template.sms_to, sms_template.model_id.model, record_id)
        gateway_model = sms_template.from_mobile_verified_id.account_id\
            .account_gateway_id.gateway_model_name
        # Queue the SMS message since we can't directly send MMS
        queued_sms = self.env['sms.message'].create(
            {'record_id': record_id, 'model_id': sms_template.model_id.id,
             ...
             'sms_content': rendered_sms_template_body, 'direction': 'O',
             'message_date': datetime.utcnow(), 'status_code': 'queued'})
        # Also create the MMS attachment
        if sms_template.media_id:
            self.env['ir.attachment'].sudo().create(
                {'name': 'mms ' + str(queued_sms.id), 'type': 'binary',
                 'datas': sms_template.media_id, 'public': True,
                 'res_model': 'sms.message', 'res_id': queued_sms.id})
    ...
    def render_template(self, template, model, res_id):
        """Render the given template text, replace mako expressions ``${expr}``
                   ...
                   an evaluation context containing:
                        ...
                        * ``object``: browse_record of the document record
                        this mail is
                                      related to
                   ...
                   :param str model: model name of the document record this
                   mail is related to.
                   :param int res_id: id of document records those mails are
                   related to.
                """
