...
class SmsGatewayTwilio(models.Model):
    ...
    def send_message(self, sms_gateway_id, from_number, to_number, sms_content,
                     my_model_name='', my_record_id=0, media=None,
                     queued_sms_message=None, media_filename=False):
        # Actual Sending of the sms
        sms_account = self.env['sms.account'].search(
            [('id', '=', sms_gateway_id)])
        ...
        if media:
            attachment_id = self.env['ir.attachment'].sudo().create(
                {'name': 'mms ' + str(my_record_id), 'type': 'binary',
                 'datas': media, 'public': True, 'mms': True,
                 'datas_fname': media_filename})
            # Force the creation of the new attachment before you make the
            # request
            request.env.cr.commit()
            if media_filename:
                media_url = base_url + "/sms/twilio/mms/" + str(
                    attachment_id.id) + "/" + media_filename
            else:
                media_url = base_url + "/sms/twilio/mms/" + str(
                    attachment_id.id) + "/media." + \
                            attachment_id.mimetype.split("/")[1]
        #send the sms/mms
        payload = {'From': format_from, 'To': format_to, 'Body': sms_content,
                   'StatusCallback': base_url + "/sms/twilio/receipt"}
        if queued_sms_message:
            for mms_attachment in queued_sms_message.attachment_ids:
                # For now we only support a single MMS per message but that
                # will change in future versions
                payload['MediaUrl'] = base_url + "/web/image/" + str(
                    mms_attachment.id) + "/media." + \
                                      mms_attachment.mimetype.split("/")[1]
        if media:
            payload['MediaUrl'] = media_url
        response_string = requests.post(
            "https://api.twilio.com/2010-04-01/Accounts/" + str(
                sms_account.twilio_account_sid) + "/Messages", data=payload,
            auth=(str(sms_account.twilio_account_sid),
                  str(sms_account.twilio_auth_token)))
        """Analyse the reponse string and determine if it sent successfully 
        other wise return a human readable error message"""
        human_read_error = ""
        ...
        """The message id is important for delivary reports also set 
    ...
    def check_messages(self, account_id, message_id=""):
        """Checks for any new messages or if the message id is specified get
        only that message"""
        ...
        if message_id != "":
            ...
            response_string = requests.get(
                ...
                sms_account.twilio_account_sid + "/Messages/" + message_id,
                data=payload, auth=(str(sms_account.twilio_account_sid),
                                    str(sms_account.twilio_auth_token)))
            ...
            # only get the inbound ones as we track the outbound ones back
            # to a user profile
            ...
            if sms_account.twilio_last_check_date != False:
                my_time = sms_account.twilio_last_check_date
                payload = {'DateSent>': str(my_time.strftime('%Y-%m-%d'))}
            response_string = requests.get(
                "https://api.twilio.com/2010-04-01/Accounts/" +
                sms_account.twilio_account_sid + "/Messages",
                data=payload, auth=(str(sms_account.twilio_account_sid),
                                    str(sms_account.twilio_auth_token)))
            root = etree.fromstring(response_string.text.encode("utf-8"))
            #get all pages
            ...
            while True:
                ...
                for sms_message in my_messages:
                    """only get the inbound ones as we track the outbound 
                    ones back to a user profile"""
                ...
                if next_page_uri != "":
                    response_string = requests.get(
                        "https://api.twilio.com" + messages_tag[0].attrib[
                            'nextpageuri'], data=payload, auth=(
                        str(sms_account.twilio_account_sid),
                        str(sms_account.twilio_auth_token)))
                    root = etree.fromstring(
                        response_string.text.encode("utf-8"))
                    messages_tag = root.xpath('//Messages')
    ...
    def _add_message(self, sms_message, account_id):
        ...
        my_message = self.env['sms.message'].search(
            [('sms_gateway_message_id', '=', sms_message.find('Sid').text)])
        if len(my_message) == 0 and sms_message.find(
                'Direction').text == "inbound":
            target = self.env['sms.message'].find_owner_model(sms_message)
            twilio_gateway_id = self.env['sms.gateway'].search(
                [('gateway_model_name', '=', 'sms.gateway.twilio')])
            discussion_subtype = self.env['ir.model.data'].get_object('mail',
                                                                'mt_comment')
            my_message = ""
            attachments = []
            ...
            if int(sms_message.find('NumMedia').text) > 0:
                ...
                for sub_resource in sms_message.find('SubresourceUris'):
                    ...
                    media_response_string = requests.get(
                        "https://api.twilio.com" + media_list_url, auth=(
                        str(sms_account.twilio_account_sid),
                        str(sms_account.twilio_auth_token)))
                    media_root = etree.fromstring(
                        media_response_string.text.encode("utf-8"))
                    for media_mms in media_root.xpath('//MediaList/Media'):
                        ...
                        attachments.append((media_filename, requests.get(
                        "https://api.twilio.com" + first_media_url).content))
            from_record = self.env['res.partner'].sudo().search(
                [('mobile', '=', sms_message.find('From').text)])
            if from_record:
                message_subject = "SMS Received from " + from_record.name
            else:
                message_subject = "SMS Received from " + sms_message.find(
                    'From').text
            if target['target_model'] == "res.partner":
                model_id = self.env['ir.model'].search(
                    [('model', '=', target['target_model'])])
                my_record = self.env[target['target_model']].browse(
                    int(target['record_id'].id))
                my_message = my_record.message_post(
                    body=sms_message.find('Body').text,
                    subject=message_subject, subtype_id=discussion_subtype.id,
                    author_id=my_record.id, message_type="comment",
                    attachments=attachments)
                """Notify followers of this partner who are listenings to 
                the 'discussions' subtype"""
                for notify_partner in self.env['mail.followers'].search(
                        [('res_model', '=', 'res.partner'),
                         ('res_id', '=', target['record_id'].id),
                         ('subtype_ids', '=', discussion_subtype.id)]):
                    my_message.needaction_partner_ids = [
                        (4, notify_partner.partner_id.id)]
                #Create the sms record in history
                history_id = self.env['sms.message'].create(
                    {'account_id': account_id, 'status_code': "RECEIVED",
                     'from_mobile': sms_message.find('From').text,
                     'to_mobile': sms_message.find('To').text,
                     'sms_gateway_message_id': sms_message.find('Sid').text,
                     'sms_content': sms_message.find('Body').text,
                     'direction': 'I',
                     'message_date': sms_message.find('DateUpdated').text,
                     'model_id': model_id.id,
                     'record_id': int(target['record_id'].id),
                     'by_partner_id': my_record.id})
                ...
                """Notify followers of this lead who are listenings to the 
                'discussions' subtype"""
                for notify_partner in self.env['mail.followers'].search(
                        [('res_model', '=', 'crm.lead'),
                         ('res_id', '=', target['record_id'].id),
                ...
                # Create the sms record in history without the model or
                # record_id
                history_id = self.env['sms.message'].create(
                    {'account_id': account_id, 'status_code': "RECEIVED",
                     'from_mobile': sms_message.find('From').text,
    ...
    def delivary_receipt(self, account_sid, message_id):
        """Updates the sms message when it is successfully received by the
        mobile phone"""
        my_account = self.env['sms.account'].search(
            [('twilio_account_sid', '=', account_sid)])[0]
        response_string = requests.get(
            "https://api.twilio.com/2010-04-01/Accounts/" +
            my_account.twilio_account_sid + "/Messages/" + message_id,
            auth=(str(my_account.twilio_account_sid),
                  str(my_account.twilio_auth_token)))
        parser = etree.XMLParser(ns_clean=True, recover=True,
                                     encoding='utf-8')
        root = etree.fromstring(response_string.text.encode('utf-8'), parser=parser)
        ...
        if root.xpath('//Status')[0].text == "failed":
            ...
            delivary_state = "UNDELIV"
        my_message = self.env['sms.message'].search(
            [('sms_gateway_message_id', '=', message_id)])
        if len(my_message) > 0:
            my_message[0].status_code = delivary_state
            my_message[0].delivary_error_string = root.xpath('//ErrorMessage')[
                0].text
...
class SmsAccountTwilio(models.Model):
    ...
    _description = "Adds the Twilio specfic gateway settings to the sms " \
                   "gateway accounts"
...
#     @api.one
    def twilio_quick_setup(self):
        """Configures your Twilio account so inbound messages point to your
        server, also adds mobile numbers to the system"""
        response_string = requests.get(
            "https://api.twilio.com/2010-04-01/Accounts/" +
            self.twilio_account_sid,
            auth=(str(self.twilio_account_sid), str(self.twilio_auth_token)))
        if response_string.status_code == 200:
            response_string_twilio_numbers = requests.get(
                "https://api.twilio.com/2010-04-01/Accounts/" +
                self.twilio_account_sid + "/IncomingPhoneNumbers",
                auth=(
                str(self.twilio_account_sid), str(self.twilio_auth_token)))
            # go through each twilio number in the account and set the the
            # sms url
            root = etree.fromstring(
                response_string_twilio_numbers.text.encode("utf-8"))
            ...
            for my_from_number in my_from_number_list:
                ...
                if self.env['sms.number'].search_count(
                        [('mobile_number', '=', av_phone)]) == 0:
                    vsms = self.env['sms.number'].create(
                        {'name': av_phone, 'mobile_number': av_phone,
                         'account_id': self.id})
                payload = {'SmsUrl': str(
                    request.httprequest.host_url + "sms/twilio/receive")}
                requests.post(
                    "https://api.twilio.com/2010-04-01/Accounts/" +
                    self.twilio_account_sid + "/IncomingPhoneNumbers/" + sid,
                    data=payload, auth=(
                    str(self.twilio_account_sid), str(self.twilio_auth_token)))
