...
class documents_document(models.Model):
    ...
    def check_all_btn(self):
        """
        Checl that files might be uploaded in archive
        ...
        res = False
        for doc in self:
            if doc.type == "binary" or \
                    (doc.cloud_key and doc.mimetype not in ['application/octet-stream', 'special_cloud_folder']):
                res = True
                break
        return res
