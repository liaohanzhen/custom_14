        ...
        for rec in self:
            if rec.move_type in ['in_invoice', 'in_refund']:
                seller_payment = self.env["seller.payment"].search(
                    [("invoice_id", "=", rec.id)])
                if seller_payment and seller_payment.payment_mode == "cod_payment" and rec.payment_state == "paid":
        ...
        if invoice_id:
            ...
            if transaction_id and transaction_id.acquirer_id and transaction_id.acquirer_id.provider == 'cash_on_delivery':
                ...
                for seller in sellers_dict["seller_ids"].keys():
                    ...
                    # journal_id = seller_payment_id.invoice_id.journal_id if seller_payment_id.invoice_id else False
                    ...
                    # _logger.info("=====================TESTING+++++++++++++%r",[journal_id.read()])
                    # payment_vals = {
                    #     'payment_type': "outbound",
                    #     'partner_type': "supplier",
                    #     'partner_id': seller_payment_id.seller_id.id,
                    #     # 'communication': seller_payment_id.name,
                    ...
                    #     'move_id': seller_payment_id.invoice_id.id,
                    #     'amount': abs(seller_payment_id.payable_amount),
                    # }
                    # payment_obj = self.env['account.payment'].create(payment_vals)
                    # payment_obj.post()
                    # seller_payment_id.write({'state': "posted"})
        return result
