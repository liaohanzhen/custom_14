...
"use strict";
    ...
    BasicRenderer.include({
                                    ...
                                    $(fields).each(function(){
                                        ...
                                        if($inputs.length > 1) {
                                            ...
                                            if($input_from.val() && $input_to.val()) {
                                                ...
                                                var date_from_format = moment($input_from.val()).locale('en').format(date_format),
                                                    date_to_format = moment($input_to.val()).locale('en').format(date_format);
                                            ...
                                            if(value){
                                                if($inputs.data('field-type') == 'boolean') {
                                                    ...
                                                    if($inputs.data('field-type') == 'date') {
                                                        var date_format = 'YYYY-MM-DD';
                                                        value = moment($inputs.val()).add(-self.getSession().getTZOffset($inputs.val()), 'minutes');
                                                        value = value.locale('en').format(date_format);
        ...
        async _render() {
            ...
            var self = this;
            if(self.viewType == 'list' && self.hasSelectors) {
                self._renderQuickSearch();
            }
            if(self.viewType == 'kanban' && self.state.count > 1) {
                self._renderQuickSearch();
            }
        }
    });
