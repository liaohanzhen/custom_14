...
class knowsystem_section(models.Model):
    ...
    _inherit = ["knowsystem.node", "mail.activity.mixin", "mail.thread"]
    ...
    def write(self, vals):
        res = super(knowsystem_section, self).write(vals)
        articles = self.env['knowsystem.article'].search([('section_id', '=', self.id)])
        for article in articles:
            self.env['mail.followers']._insert_followers('knowsystem.article', [article.id], 
                self.message_follower_ids.ids, False, False, False, customer_ids=None, check_existing=True, existing_policy='skip')
        return res
