...
class knowsystem_article(models.Model):
    ...
    def create(self, values):
        ...
        new_followers = []
        for follower in res.section_id.message_follower_ids:
            new_followers.append(follower.partner_id.id)
        if new_followers:
            self.env['mail.followers']._insert_followers('knowsystem.article', [res.id], 
                new_followers, False, False, False, customer_ids=None, check_existing=True, existing_policy='skip')
        res.message_subscribe(new_followers, None)
        res._message_auto_subscribe_notify(new_followers, 'mail.message_user_assigned')
        return res
    ...
    def action_make_template(self):
        ...
        action = action_id.read()[0]
        action["context"] = {"default_knowdescription": self.with_context(lang=self.env.user.lang).description}
        return action
