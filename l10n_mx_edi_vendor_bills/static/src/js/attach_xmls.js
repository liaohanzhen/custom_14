odoo.define('l10n_mx_edi_vendor_bills.attach_xmls', function (require) {
    "use strict";
...
//    debugger;
    ...
    var attachXmlsWizard = FieldChar.extend({
        ...
        events: {
            ...
            'click #alertscontent .dnd-alert-remove-all-wrong-xml-button': function (e) {
                debugger;
                var alertnode = e.currentTarget.parentElement.parentElement;
                ...
                $.each(self.alerts_in_queue.alertHTML, function (filekey, file) {
                    if (file.alert[0].innerText.includes('The XML UUID</span> belong to other invoice')) {
                    ...
                    else if (file.alert[0].innerText.includes('The invoice reference</span> belong to other')) {
                        if (filekey_current == filekey) {
                            remove_current_file = true;
                        }
                        else {
            ...
            // Shows documents created in a treeview
        ...
        prepareWrongAlert: function (key, data) {
            ...
            if (able_buttons.includes('supplier') && !able_buttons.includes('remove')) {
            ...
            if (buttons.length > 0) {
                buttons += _t('<button class="dnd-alert-remove-all-wrong-xml-button" style="background-color: #31708f; border: 0px solid; margin: 0 5px 0 0; color: #ffffff;" tag="remove">Remove All Wrong XML</button>');
            }
        ...
        wrongMsgXml: function (file, able_buttons) {
            ...
            var map_error = {
                ...
                /*cancelled: _t('<div><span level="1">The XML state</span> is CANCELLED in the SAT system.</div>'),*/
            ...
            $.each(file, function (ikey, val) {
                ...
                if (ikey === 'supplier') {
                    ...
                    able_buttons.push('tryagain');
                } else if (ikey === 'amount_tax') {
                    errors += _t('<div><span level="1">The XML tax total amount</span> does not match with <span level="1">Invoice tax total amount</span>: ') +
                        _t('XML tax total: <span level="2">') + val[0] + _t(', </span> Invoice Tax Total: <span level="2">') + val[1] + '</span> Ref: ' + val[2] + '</div>';
                    able_buttons.push('tryagain');
                } else if (ikey === 'uuid_duplicated') {
                    errors += _t('<div><span level="1">The XML UUID</span> belong to other invoice. <span level="1">UUID: </span>') + val + '</div>';
                } else if (ikey === 'reference_multi') {
                    errors += _t('<div><span level="1">The XML reference</span> matches another invoice reference. <span level="1">Partner: </span>') + val[0] + _t('<span level="1"> Reference: </span>') + val[1] + '</div>';
                ...
                } else if (ikey === 'no_xml_related_uuid') {
                    errors += _t('<div><span level="1">The DocumentType is "E" and The node CFDI related is not set</span> Manually reconcile with the appropiate invoice. <span level="1">UUID: </span>') + val + '</div>';
                    able_buttons.push('tryagain');
        ...
        sendErrorToServer: function (xml64, key, function_def) {
            /* Sends again the base64 file string to the server to retry to create the invoice, or
